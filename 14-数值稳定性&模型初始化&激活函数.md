...
$$\sigma(x)=\max(0,x)\quad and\qquad \sigma\prime(x)={\begin{cases}1&if\ x\gt0\\0&otherwise\end{cases}}$$
...
- 在训练开始的时候更容易有数值不稳定（梯度大）
  ...
  - 最优解附近表面会比较平
- 使用$\aleph(0, 0.01)$来初始可能对小网络没问题，但不能保证深度神经网络也适用

### 1. 以无激活函数的 MLP 为例，在神经网络第$t$层：

- **假设 1：** $w^t_{i,j}$是遵从独立同分布的（i.i.d），那么$\mathbb E[w^t_{i,j}]=0$，令$Var[w^t_{i,j}]=\gamma_t$
- **假设 2：** $h^{t-1}_i$独立于${w_{i,j}^t}$（即当前层的输入独立于当前层的权重）
- **假设 3：** 为推导简便，假设没有激活函数，则$\bf h^t=W^th^{t-1}$，${\bf W^t}\in\mathbb R^{n_t\times n_{t-1}}$
...
$$
\begin{aligned}
  \mathbb E[h_i^t]&=\mathbb E\left[\sum_j w_{i,j}^t h_j^{t-1}\right] \\
...
\begin{aligned}
  ...
  &=\mathbb E\left[\left({\partial l\over\partial{h_i^t}}\right)\left({\partial {w_{i,j}^{t-1} h_j^{t-1}}\over\partial{h_i^{t-1}}}\right)\right]\\
  &=\mathbb E\left[\left({\partial l\over\partial{h_i^t}}\right) \right] \mathbb E\left[w_{i,j}^{t-1} \right] \quad \quad(14.5) \\
  &=0
\end{aligned}
$$

> 由假设 1 可知，(14.5)式中$\mathbb E[w_{i,j}^{t-1}]=0$
>
...
\begin{aligned}
  ...
  &=Var\left[\left({\partial l\over\partial{h_i^t}}\right)\left({\partial {w_{i,j}^{t-1}h_j^{t-1}}\over\partial{h_i^{t-1}}}\right)\right]\\
  &=Var \left[{\partial l\over\partial{h_i^t}}\right] Var\left[w_{i,j}^{t-1} \right] \\
  &=Var \left[{\partial l\over\partial{h_i^t}}\right] \gamma_{t-1} \quad (14.6)\\
  &=n_t \gamma_tVar\left[{\partial l\over\partial{h_i^t}}\right] \quad (14.7) \\
...
- $n_{t-1}\gamma_t=1$（即每一个前向输出方差保持一致的条件）
- $n_t\gamma_t=1$（即每个梯度方差保持一致的条件）

### 达到以上目的的方法——Xavier 初始化（随机初始化）

由以上推导可知，要想满足每层神经网络输出和梯度的方差均为常数，则初始化权重时设置的方差$\gamma_t$的选择是根据输入维度$n_{t-1}$和输出维度$n_t$来确定的。

但是难以取得一个$\gamma_t$，使之同时满足$n_{t-1}\gamma_t=1$和$n_t\gamma_t=1$。因为$n_{t-1}$和$n_t$的值是不可控制的。除非输入维度等于输出维度。
...
$$relu(x)=0+x\quad for \ x\ge0$$

![激活函数在零点附近的取值](Images\3-Figure1-1.png)

> 蓝色线代表的`scaled sigmoid`函数，是通过下列变换得到：
> $$scaled\ sigmoid(x)=4\times\sigma(x)-2$$

$tanh(x),relu(x)$在零点附近，存在近似区间使得$f(x)=x$，而$sigmoid(x)$则不行。

通过线性变换（伸缩、平移变换），可以使 sigmoid 在零点附近近似成恒等函数，此时 sigmoid 函数相比其他激活函数原有的缺点将得到改善。
