...
通常为方便计算机使用并行加速运算能力，会**向量化**训练数据表示为列向量，相关理论可参考吴恩达Deeplearning课程第17节内容👉[Bilibili](https://www.bilibili.com/video/BV1FT4y1E74V?p=17)，形式为：
$$
...
- **损失函数**（Loss function）：用于衡量预估质量，通常我们会选择非负数作为损失，且数值越小表示损失越小，完美预测时的损失为0。 回归问题中最常用的损失函数是平方误差函数。下式表示每个样本的损失：
$$
l(y,\hat{y})={1\over2}{(y-\hat{y})^2}
$$
（其中，式中的$1\over2$方便求导）

![](http://zh.d2l.ai/_images/fit-linreg.svg)
...
- 显式解：线性回归问题存在最优的解析解
  ...
  首先将特征矩阵增加一列全1向量，再将偏差`b`合并进权重 **`w`**，以简化表示：
  $$
...
$b$为批量大小（batch），另外一个重要的超参数。*为最大化效率，一般与运算设备（如GPU）的存储大小相关，如256、512、2048……*
...
- `torch.nn`神经网络基本Block，如全连接层、卷积层、损失函数等等的实现🧐[中文](https://pytorch-cn.readthedocs.io/zh/latest/package_references/torch-nn/) | [官方英文](https://pytorch.org/docs/stable/nn.html)
...
**A**：本地图片也可以插入，但首先要保证图片存储于代码仓库中，即保证Markdown可以访问得到，Markdown插入图片语法为：

`![图片名称](图片资源URL地址)`

我在代码仓库根目录创建了名为“**Images**”的文件夹，把需要插入的本地图片先复制到此文件夹中，再在URL地址填写：`Images/xxx.jpg`即可。以下图片便是引用Images文件夹中的`test_img.png`图片文件👇
